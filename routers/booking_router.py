...
@router.get("/detailed/by-month", response_model=list[schema.BookingDetailedResponse])
def get_detailed_bookings_by_month(year: int, month: int, db: Session = Depends(get_db)):
    return service.get_detailed_bookings_by_month(db, year, month)

@router.get("/detailed/by-date/{target_date}", response_model=list[schema.BookingDetailedResponse])
def get_detailed_bookings_by_date(target_date: date, db: Session = Depends(get_db)):
    return service.get_detailed_bookings_by_date(db, target_date)
